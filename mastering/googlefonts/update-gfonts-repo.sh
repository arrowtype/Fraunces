...
frauncesRomanVF="fonts/Fraunces[SOFT,WONK,opsz,wght].ttf"
frauncesItalicVF="fonts/Fraunces-Italic[SOFT,WONK,opsz,wght].ttf"

staticsDir="fonts/static/ttf" # will be copied to directory ofl/fraunces/static
...
# Path to local fraunces directory
frauncesDir=$(pwd)
...
# if argument is missing or is "--help"
if [[ -z "$pushToGitHub" ]] ; then
    echo 'Dry run. Add argument --push to actually push to google/fonts'
fi
...
# copy variable fonts
cp $frauncesDir/$frauncesRomanVF    ofl/$gFontsSubDir/$(basename $frauncesRomanVF)
cp $frauncesDir/$frauncesItalicVF   ofl/$gFontsSubDir/$(basename $frauncesItalicVF)

# copy static fonts
cp -r $frauncesDir/$staticsDir ofl/$gFontsSubDir/static
